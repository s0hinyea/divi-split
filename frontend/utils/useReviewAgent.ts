...
export function useReviewAgent(
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      messagesRef.current = [
        ...messagesRef.current,
        { id: `${Date.now()}-u`, role: "user", content: text.trim() },
      ];
      ...
      try {
        ...
        messagesRef.current = [
          ...messagesRef.current,
          { id: `${Date.now()}-a`, role: "assistant", content: reply || "Done!" },
        ];
...
export function executeMoveItem(itemId: string, fromId: string, toId: string) {
  ...
  if (fromId === "user") {
    ...
    if (item && fromContact) store.manageItems(item, fromContact);
  }
  ...
  if (toId === "user") {
    ...
    if (toContact) fresh.manageItems(item, toContact);
  }
}
