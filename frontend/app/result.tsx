...
export default function OCRResults() {
	...
	const { updateItem, removeItem, addItem, updateReceiptData, receiptData } =
		useReceipt();
	...
	const [taxInput, setTaxInput] = useState<string>(
		receiptData && "tax" in receiptData && receiptData.tax !== undefined
			? receiptData.tax.toString()
			: ""
	);
		...
		<View style={styles.container}>
			<ScrollView style={styles.scrollView}>
				...
				<View style={styles.itemsContainer}>
					{items
						.filter(
							(item) => item.name.trim().toLowerCase() !== "tax"
						)
						.map((item) =>
							changing === item.id ? (
								...
								<Swipeable
									key={item.id}
									renderRightActions={() =>
										renderRightActions(item.id, item)
									}
									rightThreshold={20}
								>
									<TouchableOpacity
			...
			<Modal
				...
				transparent={true}
				visible={adding}
				onRequestClose={() => isAdding(false)}
			>
				<BlurView intensity={50} style={styles.modalOverlay}>
					<Pressable
						style={styles.modalOverlay}
						onPress={() => isAdding(false)}
					>
